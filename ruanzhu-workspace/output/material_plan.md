...
> **rev1（2026-09-06，Step 2 实施期勘误——机械精度修正，不改已确认的文件顺序与前后 30 页策略）**
> ① `sqlite_repository.dart` 含三引号多行 SQL，清洗器按"元素=物理行"不变量逐行拆分：238→**246**，全量 5780→**5788**（表内第 12~20 项累计值 +8，后 30 页起点相应约移至 4289）。
> ② Word 排版下超宽行（>97 半角字符 @9pt）会不可控软换行，故产物以"**换行流**"（保守 53 单位/行显式断行，不增删任何字符）取前 1500 + 末 1500 **物理行** = 恰 3000 行 ≡ 60 页 × 50 行，页页精确。实际断点由 build 运行日志固化（含换行流段映射）。
