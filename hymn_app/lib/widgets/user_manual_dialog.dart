...
class UserManualDialog extends StatelessWidget {
  ...
  Widget _buildGuideList(BuildContext context) {
    const items = <String>[
      '本手册的打开与关闭：点击标题栏右侧「？」按钮（最小化左侧），或在软件任意界面按 F1；关闭可按 Esc、点右上角 ✕ 或点弹窗外任意处。每次启动软件会自动弹出本手册，可在弹窗底部勾选/取消「启动时显示本手册」来控制。',
      '标题栏：空白处按住拖动可移动窗口，双击最大化 / 还原；右侧按钮组从左至右依次为「调色盘」配色、「大小写T」字号、「？」用户手册、最小化、最大化·还原、关闭。',
      '字号：「大小写T」图标按钮（位于调色盘与手册之间）提供默认 / 中号 / 大号 / 最大四档，全界面控件与文字等比例放大、即时生效；选择自动记忆，重启保持。歌词区按显示区域自动铺满，放大后歌词超出显示区时可滚动阅读。',
      ...
      '搜索：在搜索框输入后按回车触发——输入纯数字按编号定位并翻页高亮（再按一次回车播放）；输入中文同时模糊搜索歌名与歌词，有命中时弹出统一结果列表（编号 / 诗歌名称 / 歌词三列）：歌名命中的关键字红色加粗、歌词列默认显示第一节；歌词命中显示第一个命中的歌词节、关键字主题色加粗；同一首两种命中在同一行并存。弹窗内单击行选中、双击行即在左栏定位该诗歌并播放；无命中时列表显示「未找到匹配的诗歌」。清除（×）保持当前定位。',
      '音频版本：主内容区顶部「钢琴版｜人声版」切换音频版本；某首诗歌有多个（>1）人声版本时，播放条右侧出现 👥 按钮可选择切换。',
      '显示模式：「歌词」「简谱」「五线谱」切换主内容区显示内容（简谱 / 五线谱可缩放拖动查看）。',
      '播放条：进度条可拖动跳转，两端显示播放位置与总时长；上一首 / 播放·暂停 / 下一首 三个控制键；左侧音量控件——点击图标静音或取消、拖动滑条调节、右侧显示音量百分比。音量滑条与 Windows 系统音量同步（相当于系统音量条在应用内的镜像，两者听感一致）。',
