...
/// 操作说明内容（按界面区域分小节、逐子项说明；无版本化措辞）
const List<({String title, List<String> lines})> _guideSections = [
  (
    title: '本手册的打开与关闭',
    lines: [
      '打开：点击标题栏右侧「？」按钮（最小化左侧）；或在软件任意界面按 F1；每次启动软件默认自动弹出。',
      '关闭：按 Esc、点弹窗右上角 ✕、或点弹窗外任意位置。',
      '弹窗底部的「启动时显示本手册」勾选框可控制每次启动是否自动弹出。',
    ],
  ),
  (
    title: '标题栏与窗口',
    lines: [
      '标题栏空白处按住拖动可移动窗口，双击最大化 / 还原。',
      '右侧按钮组从左至右：「调色盘」配色、「大小写T」字号、「？」用户手册、最小化、最大化·还原、关闭。',
      '改变窗口大小时界面内容整体等比缩放，不变形不裁切。',
    ],
  ),
  (
    title: '字号',
    lines: [
      '「大小写T」按钮（位于调色盘与手册之间）提供 默认 / 中号 / 大号 / 最大 四档，当前档带 ✓。',
      '全界面控件与文字（含弹窗）等比例放大，点击即时生效。',
      '歌词区按显示区域自动铺满；放大后歌词超出显示区时可滚动阅读。',
      '字号选择自动记忆，重启保持。',
    ],
  ),
  (
    title: '配色',
    lines: [
      '「调色盘」按钮提供五套界面配色：晨光蓝 · 经典 / 暖阳金 · 圣堂 / 静谧绿 · 草木 / 典雅紫 · 暮云 / 暗夜墨 · 深色。',
      '标题栏、顶栏、侧栏、歌词区等各分区底色自动配套为同色相层次，点击即时生效。',
      '配色选择自动记忆，重启保持。',
    ],
  ),
  (
    title: '顶栏',
    lines: [
      '左端箭头按钮：展开 / 收起左侧栏；右端箭头按钮：展开 / 收起右侧栏。',
      '中央显示当前选中歌曲（第 N 首《标题》），切歌即时刷新。',
    ],
  ),
  (
    title: '左侧栏（三个栏目）',
    lines: [
      '「诗歌列表」：按编号分页浏览全部 474 首诗歌，每页 35 首，底部可翻页。',
      '「默认歌单」：按分类目录两级展开浏览，点击诗歌播放。',
      '「个人歌单」：可新建 / 编辑 / 删除自己的歌单，点歌单名展示并整单播放。',
      '正在播放的诗歌在列表中高亮，切歌时自动滚动跟随。',
    ],
  ),
  (
    title: '搜索（诗歌列表栏目）',
    lines: [
      '输入纯数字回车：按编号翻页定位并高亮该诗歌（不自动播放）；再按一次回车开始播放，播完搜索框自动清空。',
      '输入中文回车：同时模糊搜索歌名与歌词，有命中时弹出统一结果列表（编号 / 诗歌名称 / 歌词三列）。',
      '歌名命中：名称中关键字红色加粗，歌词列默认显示第一节。',
      '歌词命中：歌词列显示第一个命中的歌词节（从关键字所在行开始），节内关键字蓝色加粗。',
      '同一首歌两种命中都有时，在同一行红、蓝并存显示。',
      '弹窗中单击行选中，双击行即关闭弹窗、在左栏定位该诗歌并播放；回车不会直接播放。',
      '无命中时不弹窗，列表显示「未找到匹配的诗歌」；点搜索框清除（×）回到当前播放定位行。',
    ],
  ),
  (
    title: '音频版本',
    lines: [
      '主内容区顶部「钢琴版｜人声版」按钮切换播放版本。',
      '某首诗歌有多个（多于一个）人声版本时，播放条右侧出现 👥 按钮可选择。',
      '版本选择自动记忆；某首没有所选版本时临时改用另一版本播放，记忆不变。',
    ],
  ),
  (
    title: '显示模式',
    lines: [
      '「歌词」「简谱」「五线谱」三个按钮切换主内容区显示内容。',
      '简谱 / 五线谱图片支持缩放与拖动查看。',
    ],
  ),
  (
    title: '播放条与音量',
    lines: [
      '进度条可拖动跳转，两端显示播放位置与总时长。',
      '控制键：上一首 / 播放·暂停 / 下一首，列表内循环切换。',
      '左侧音量控件：点击图标静音或取消、拖动滑条调节、右侧显示音量百分比。',
      '音量滑条与 Windows 系统音量同步（滑条即系统音量的镜像，与系统播放器听感一致）。',
    ],
  ),
  (
    title: '右侧栏（诗歌源考）',
    lines: [
      '显示当前诗歌的源考资料（诗歌背景、词曲作者信息等）。',
      '内容超过一屏时可滚动阅读；经顶栏右端箭头展开 / 收起。',
    ],
  ),
  (
    title: '底部状态栏',
...
class UserManualDialog extends StatelessWidget {
  ...
  /// 操作说明：按界面区域分小节，每小节拆分项逐条说明（避免整段混排）
