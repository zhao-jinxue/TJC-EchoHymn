...
const List<({String title, List<GuideLine> lines})> _guideSections = [
  ...
  (
    ...
    lines: [
      ...
      GuideLine('「曲谱」= 整页简谱网格：谱面 + 全部节歌词并列（同印刷本/投影版式）；', [
        '数据为「列 = 拍点」的谱面网格：音符、记号、小节线与歌词同列对齐；',
        '不分节、无翻页条，内容超出时上下滚动。',
      ]),
      GuideLine('「简谱」= 一页一节的简谱网格：', [
        '每页只显示当前节的歌词行（谱面各节共用，不随节变化）；',
        '底部翻页条：◀ / 第 N / M 节 / ▶ 切换节；',
