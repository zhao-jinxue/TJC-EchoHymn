...
void main() {
  ...
  test('重复点击同一字号等级，系数固定不变（不会连续变大）', () {
    ...
    expect(FontScaleController.instance.scale, 1.4);

    // 连续重复点击「大号」→ 守卫直接 return，系数始终 1.4（绝非 1.4×1.4 累乘）
    for (var i = 0; i < 5; i++) {
      FontScaleController.instance.switchTo(FontSizeLevel.large);
      expect(FontScaleController.instance.scale, 1.4,
          reason: '第 ${i + 1} 次重复点击后系数必须仍是 1.4');
    }

    // 切走再切回 → 仍是固定 1.4
    FontScaleController.instance.switchTo(FontSizeLevel.medium);
    expect(FontScaleController.instance.scale, 1.2);
    FontScaleController.instance.switchTo(FontSizeLevel.large);
    expect(FontScaleController.instance.scale, 1.4);
  });
  ...
  test('四档字号系数均为固定常量', () {
    ...
    expect(FontSizeLevel.medium.scale, 1.2);
    expect(FontSizeLevel.large.scale, 1.4);
    expect(FontSizeLevel.xlarge.scale, 1.6);
  });
