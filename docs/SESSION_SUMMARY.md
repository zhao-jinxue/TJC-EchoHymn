...
- 字段：`leftTab` / `subcategory` / `playlistName` / `hymnNumber` / `audioVersion` / `displayMode` / `playlistIndex` / **`showLeft`** / **`showRight`** / **`appTheme`**（v1.4.0 配色）/ **`fontSizeLevel`**（v1.5.0 字号等级）
