...
| `(v1.5.0，2026-08-31)` | **四级字号切换 + 全局等比缩放**：新增 `lib/theme/app_fonts.dart`（`FontSizeLevel` 枚举 ×1.0/×1.3/×1.6/×1.9 + `FontScaleController` ValueNotifier + `AppFonts` 门面）；`MaterialApp.builder` 内 `Transform.scale`（复用 v1.2.x 已验证模式）把整棵 Navigator（含弹窗/菜单/Toast）等比例缩放；标题栏调色盘左侧新增「A」字号按钮（4 级菜单 + ✓）；**歌词区单独处理**（铺满自适应算法 × 系数）；**左栏宽度随系数缩放**（350×s）并同步 native 窗口扩展，**右栏宽度固定 600 不缩放**（有滚动条）；`state.json fontSizeLevel` 持久化 + 重启恢复；换肤菜单 `localToGlobal` 加 `ancestor: overlay` 兼容缩放；单元测试 `font_size_level_test.dart`。**测试后修复**：F27 音量滑条（方案A：图标/百分比固定视觉+滑轨占满可用永不小于150px+滑块轨道反向缩放）、F28 最大化盖任务栏（`WM_GETMINMAXINFO` 限工作区）、皮肤名称字重统一 w500；测试清单入 `test/v150/` 目录 + 复测清单 |
