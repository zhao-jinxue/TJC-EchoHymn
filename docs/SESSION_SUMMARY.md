...
| `v1.5.0`（**tag**，2026-09-01） | 🎉 **v1.5.0 全面验收完成**（v1.4.0 以来全部改动）：四级字号切换+全局等比缩放（R37~R42 全 OK）、播放条镜像 Row 音量重叠根治、内置字体 EchoSans（真 400/500/700 不依赖系统）、用户手册无版本化重构+启动弹出+「启动时显示本手册」开关（M01~M15 全 OK）、音量双重衰减根治（系统音量唯一主旋钮，Q01~Q08 全 OK）；含 8 个 feat/fix 提交 + 文档三件套同步 |
