...
- **模式集（2026-09-23 对调定稿，v1.7.2）**：版本栏四按钮 `歌词 / 曲谱 / 简谱 / 五线谱` ——
  `曲谱` = **一页一节**的单声部简谱网格（主旋律 + 该节一行歌词；翻页条 + 自动跟随播放）；
  `简谱` = **简谱扫描图片**（印刷四声部谱，参考图，可缩放拖动）；整页多节并列形态已移除；
  历史 state.json 的 `scoreLyric`→`score`、`numbered`→`jianpu` 归一化；
  用户手册「显示模式」与 UI_CONFIRMATION §5.21/§5.22 已同步。
- **按块宽渲染（2026-09-23，v1.7.1）**：每个乐句块用**自己的列数**铺满可用宽
  （与 APK 每表独立排布同构）；块内行宽差异（`163`/`197`/`297`）按块内最大行宽处理，
  窄行右侧留空、同列仍同 x；总高超限时整体等比收缩。golden `jianpu_163.png` 已核对。
- **单声部（2026-09-23，v1.7.3）**：APK CSV 实为**四部合唱谱**（每块 = 女高/女低/歌词/男高/男低
