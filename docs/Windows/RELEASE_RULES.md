...
> 配套文档：`docs/Windows/INSTALLER.md`（操作手册）；构建入口：`tools/build_installer.ps1`。
...
| 6 | **压缩使安装包尽量小** | **主体/素材双载荷拆分（2026-09-05）**：安装包仅内嵌主程序载荷（7z LZMA2 solid preset 1，约 65 MB）；诗歌素材 `Hymn_Downloads`（约 3 GB，媒体不可再压取物理下限）打包为**外置加密数据文件** `EchoHymn_Data_v<版本>.7z` 单独分发，安装向导检测其必须在安装包同级目录，缺失则环境检查 ✘ 阻断；打包清单过滤无引用素材（`payload_manifest.txt`，由 `tools/scan_db_refs.py` 生成）；外壳 ISCC `Comp=none`（加密数据不再压） |
| 7 | **加密使安装包不可被轻易破解** | 主载荷与外置素材载荷均为 **AES-256 + 加密头**（文件清单不可见，同一口令），密钥 XOR 混淆内嵌于向导代码（`DecodeKey`），拦截右键解压/解包工具。**能力边界**：安装包必须可被任何用户免密安装，故不宣称"不可破解"，定位为抬高门槛级防护 |
...
| # | 规则 | 实现载体 / 说明 |
| --- | --- | --- |
| 9 | **代码签名策略明示** | 首期不签名（无证书预算），SmartScreen「未知发布者」的处理指引写入 `docs/Windows/INSTALLER.md`；后续购 OV 证书则在构建脚本追加 SignTool。**历史代价与现状**：3GB 未签名单文件时代双击后有 5~15 秒系统安全扫描空档（Defender 全文件扫描+SmartScreen 哈希信誉查询）；2026-09-05 素材拆分后安装包仅约 30 MB，空档基本消除，签名收益降为消除「未知发布者」提示与批量部署信任链 |
| 10 | **兼容性检查项完整定义** | ① 64 位 Windows；② Windows 10 及以上；③ Media Foundation 可用（`{sys}\mfplat.dll`，N/KN 版须装媒体功能包，音频命脉）；④ **外置素材文件就位**（`EchoHymn_Data_v<版本>.7z` 与安装包同目录，缺失 ✘ 阻断并指引）；⑤ 磁盘空间（系统盘 ≥2GB 缓冲；安装盘 ≥ 素材+程序+5GB 缓冲，两值均编译期注入真实字节数）；⑥ VC++ 运行库随包内置说明；⑦ 旧版本检测提示（升级路径） |
...
- 更换载荷密码：改 `installer/make_payload.py` 的 `PASSWORD`，并将新口令逐字符 XOR `0x5A` 的结果同步填入 `echohymn.iss` 的 `DecodeKey`（主载荷与外置素材载荷共用同一密钥，两处必须一致）。
- 修订宣誓词：仅改 `echohymn.iss` 的 `OATH_TEXT` 常量（校验归一化逻辑随之生效），修改属规则 5 变更，需重新走查誓言页。
- **分发形态（2026-09-05 起）**：`EchoHymn_Setup_v<版本>.exe`（约 30 MB，仅内嵌主程序）+ `EchoHymn_Data_v<版本>.7z`（约 3 GB 外置素材）两文件必须置于用户电脑同一目录后双击安装包；素材不再入包使 UAC 前系统扫描空档从 ~10 秒降至 1 秒内。
