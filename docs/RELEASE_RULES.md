...
| 5 | **第三页誓言宣誓（宗教仪式关卡）** | 自定义页展示**祷告版**宣誓词（`OATH_TEXT` 常量）；展示框与输入框均拦截 复制/粘贴/全选（Ctrl+C/V/X/A、Shift+Insert）；用户须照抄逐字输入；校验=去空白+半角标点归一全角后全文比对；**失败红字提示且不开始安装**。定位：敬畏感仪式，非安全机制（拍照抄写不可防） |
...
| # | 规则 | 实现载体 / 说明 |
| --- | --- | --- |
| 9 | **代码签名策略明示** | 首期不签名（无证书预算），SmartScreen「未知发布者」的处理指引写入 `docs/INSTALLER.md`；后续购 OV 证书则在构建脚本追加 SignTool |
