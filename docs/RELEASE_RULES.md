...
| # | 规则 | 实现载体 / 说明 |
| --- | --- | --- |
| 9 | **代码签名策略明示** | 首期不签名（无证书预算），SmartScreen「未知发布者」的处理指引写入 `docs/INSTALLER.md`；后续购 OV 证书则在构建脚本追加 SignTool。**历史代价与现状**：3GB 未签名单文件时代双击后有 5~15 秒系统安全扫描空档（Defender 全文件扫描+SmartScreen 哈希信誉查询）；2026-09-05 素材拆分后安装包仅约 30 MB，空档基本消除，签名收益降为消除「未知发布者」提示与批量部署信任链 |
